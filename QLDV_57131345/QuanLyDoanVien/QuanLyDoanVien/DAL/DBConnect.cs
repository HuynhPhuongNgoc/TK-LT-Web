...
namespace QuanLyDoanVien.DAL
{
    public class DBConnect
    {
        ...
        public class TimKiemDAL : DBConnect
        {
            ...
            public void TimDV(string tdv) //string mdv,string mcd
            {
                SqlConnection Conn = DBConnect.getConnection();
                conn.Open();
                SqlCommand cmd = new SqlCommand("Tim_DV", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                // cmd.Parameters.Add("@MaDV", SqlDbType.VarChar).Value = mdv;
                cmd.Parameters.Add("@TenDV", SqlDbType.NVarChar).Value = tdv;
            cmd.ExecuteNonQuery();
            conn.Close();
            }


            /* public DataTable lookDV(string ten)

    {

        string strSQL = "SELECT MaDV,HoDV,TenDV,NgaySinh,GioiTinh,QueQuan,Email,SDT,DanToc,TonGiao,NgayVaoDoan,ChooHienNay,MaChiDoan FROM DoanVienTN WHERE HoTenSV LIKE N'%" + ten + "%'";

        DataTable dt = new DataTable();

        dt = da.GetData(strSQL);
